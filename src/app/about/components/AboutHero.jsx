const features = [
  {
    ...
    icon: (
      <svg className="h-8 w-8 text-[#B844E8]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
export default function AboutHero() {
  return (
    <section className="w-full bg-gradient-to-b from-[#000000] to-[#0D0D0D] text-white pt-32 pb-8 md:pt-40 md:pb-12">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-4" style={{ fontFamily: "Poppins, sans-serif" }}>
            About Us
          </h1>
          <p className="max-w-3xl mx-auto text-base md:text-lg text-white/90" style={{ fontFamily: "Inter, sans-serif" }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8">
          {features.map((feature) => (
            <div key={feature.num} className="bg-[#1A1A1A] p-6 rounded-xl border border-white/10 shadow-sm text-center flex flex-col items-center transition-all duration-300 hover:shadow-lg hover:-translate-y-1 hover:border-[#B844E8]/20">
              <div className="mb-4">{feature.icon}</div>
              <p className="text-sm font-semibold text-white/60 mb-2" style={{ fontFamily: "Poppins, sans-serif" }}>{`<${feature.num}>`}</p>
              <h3 className="text-lg font-bold text-white mb-2" style={{ fontFamily: "Poppins, sans-serif" }}>{feature.title}</h3>
              <p className="text-sm text-white/80" style={{ fontFamily: "Inter, sans-serif" }}>{feature.text}</p>
            </div>
          ))}
        </div>
      </div>

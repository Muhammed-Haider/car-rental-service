      ...
      <div className="space-y-6">
        {contactItems.map((item) => (
          <div key={item.label} className="flex items-start gap-4">
            ...
            <div>
              <p className="text-sm text-slate-500 font-medium" style={{ fontFamily: "Inter, sans-serif" }}>{item.label}</p>
              {item.href ? (
                <a 
                  href={item.href} 
                  className="text-base font-semibold text-slate-800 hover:text-[#B844E8] transition-colors duration-300"
                  style={{ fontFamily: "Poppins, sans-serif" }}
                >
                  {item.value}
                </a>
